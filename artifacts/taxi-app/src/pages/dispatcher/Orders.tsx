...
const STATUS_MAP: Record<string, { label: string; color: string; bg: string; border: string; dot: string }> = {
  ...
  merged:      { label: "В рейсе",   color: "text-emerald-700", bg: "bg-emerald-100", border: "border-emerald-300", dot: "bg-emerald-500" },
...
function AssignModal({ rideId, onClose, headers, onAssigned }: {
  ...
  const { data } = useGetDrivers({ status: "online" as any }, { request: { headers }, query: { refetchInterval: 5000 } });
        ...
        <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-2">
          {drivers.length === 0 ? (
            ...
            <div key={d.id} className="border border-gray-200 rounded-xl p-3 flex items-center gap-3 hover:bg-gray-50 active:bg-gray-100 transition-colors">
              <div className="w-10 h-10 md:w-9 md:h-9 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-bold text-sm shrink-0">
                {d.name.charAt(0)}
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-900 text-sm truncate">{d.name}</p>
                <p className="text-xs text-gray-500">{d.carModel} · {d.carNumber}</p>
              </div>
...
export default function Orders() {
  ...
  const counts = useMemo(() => {
    return {
      ...
      in_progress: viewFilteredRides.filter((r: any) => r.status === "in_progress" || r.status === "accepted" || r.status === "merged").length,
      waiting: viewFilteredRides.filter((r: any) => (r.status === "pending" || r.status === "offered") && !isStuck(r)).length,
  ...
  const rides = useMemo(() => {
    ...
    else if (activeFilter === "in_progress") list = list.filter((r: any) => r.status === "in_progress" || r.status === "accepted" || r.status === "merged");
    else if (activeFilter === "waiting") list = list.filter((r: any) => (r.status === "pending" || r.status === "offered") && !isStuck(r));
