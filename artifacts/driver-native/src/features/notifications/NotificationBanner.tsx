...
type Cfg = { icon: LucideIcon; bg: string; defaultTitle: string };
...
const TYPES: Record<string, Cfg> = {
  urgent_order: { icon: Zap, bg: "#ef4444", defaultTitle: "Срочный заказ" },
  order_assigned: { icon: ClipboardCheck, bg: "#18181b", defaultTitle: "Заказ назначен" },
  order_taken: { icon: AlertTriangle, bg: "#18181b", defaultTitle: "Заказ занят" },
  order_expired: { icon: AlertTriangle, bg: "#18181b", defaultTitle: "Заказ истёк" },
  marketplace_sold: { icon: ShoppingBag, bg: "#18181b", defaultTitle: "Заказ продан" },
  payment_received: { icon: DollarSign, bg: "#18181b", defaultTitle: "Платёж получен" },
  driver_blocked: { icon: ShieldBan, bg: "#ef4444", defaultTitle: "Вы заблокированы" },
  driver_unblocked: { icon: Bell, bg: "#10b981", defaultTitle: "Доступ восстановлен" },
  news: { icon: Bell, bg: "#1FBAD6", defaultTitle: "Новость" },
};
...
export function NotificationBanner() {
  ...
  useEffect(() => {
    return wsEvents.on((d: any) => {
      ...
      if (!cfg) return;
      const title = d.title || cfg.defaultTitle;
